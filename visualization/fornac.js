function FornaContainer(h, f) {
  // updateViewport 
  function k() {
    var containerWidth = $(h).width(),
        containerHeight = $(h).height();
    d.options.svgW = containerWidth;
    d.options.svgH = containerHeight;
    D.range([0, containerWidth]).domain([0, containerWidth]);
    E.range([0, containerHeight]).domain([0, containerHeight]);
    zoomer.x(D).y(E);
    d.brusher.x(D).y(E);
    A.attr("width", containerWidth).attr("height", containerHeight);
    u.attr("width", containerWidth).attr("height", containerHeight);
    d.center_view();
  }

  // formatPropertyValue
  function b(dataObject, target, formatCallback) {
    return dataObject.hasOwnProperty(target.num)
      ? ((val = parseFloat(dataObject[target.num])), isNaN(val) ? dataObject[target.num] : formatCallback(val))
      : "white";
  }

  // getSelectedNodes
  function e(nodeGroups) {
    nodeGroups = p.selectAll("g.gnode");
    return nodeGroups.filter(function (nodeData) {
      return nodeData.selected;
    });
  }

  // createCollisionHandler
  function g(targetNode) {
    var padding = targetNode.radius + 16,
        minX = targetNode.x - padding,
        maxX = targetNode.x + padding,
        minY = targetNode.y - padding,
        maxY = targetNode.y + padding;

    return function (quadNode, nodeMinX, nodeMinY, nodeMaxX, nodeMaxY) {
      if (quadNode.point && quadNode.point !== targetNode) {
        const neighborNode = quadNode.point;
        var dx = targetNode.x - neighborNode.x,
            dy = targetNode.y - neighborNode.y,
            dist = Math.sqrt(dx * dx + dy * dy),
            minDist = targetNode.radius + neighborNode.radius;

        if (dist < minDist) {
          dist = ((dist - minDist) / dist) * 0.1;
          targetNode.x -= dx *= dist;
          targetNode.y -= dy *= dist;
          neighborNode.x += dx;
          neighborNode.y += dy;
        }
      }
  ...
  // handleKeyDown
  function n() {
    const isInputDisabled = d.deaf;
    if (!isInputDisabled && !C) {
      key_is_down = true;
  ...
  if (1 < arguments.length) {
    for (var w in f) {
      d.options.hasOwnProperty(w) && (d.options[w] = f[w]);
    }
  }
  ...
  d.createInitialLayout = function (b, d) {
    var e = {
      ...
      labelInterval: 10,
      avoidOthers: !0,
      circular: !1 // default Linear (False)
    };

    if (2 == arguments.length) {
      for (var g in d) {
        e.hasOwnProperty(g) && (e[g] = d[g]);
      }
    }
      ...
      g.style("fill", function (e) {
        return "undefined" == typeof d.customColors
          ...
          : d.customColors.color_values.hasOwnProperty(e.struct_name) &&
            d.customColors.color_values[e.struct_name].hasOwnProperty(e.num)
          ? ((molecule_colors = d.customColors.color_values[e.struct_name]),
  ...
  var x = u
    ...
    .on("mouseup", function () {
      if (m) H.attr("class", "drag_line_hidden");
      q = m = null;
    });
  ...
  d.center_view = function () {
    if (0 !== d.graph.nodes.length) {
      (min_x = d3.min(
        ...
        (x_trans =
          -min_x * min_ratio + (d.options.svgW - new_mol_width) / 2),
        (y_trans =
          -min_y * min_ratio + (d.options.svgH - new_mol_height) / 2),
        v.attr(
          "transform",
      ...
      if (((q = b), q == m)) {
        ...
        for (i = 0; i < d.graph.links.length; i++) {
          ...
          if (
            ...
            if ("backbone" == d.graph.links[i].link_type) return;
          }
        }
  ...
  f.parseRange = function (f) {
    for (var b = f.split(","), e = [], g = 0; g < b.length; g++) {
      ...
      if (1 == n.length) {
        ...
        for (var l = parseInt(n[0]), n = parseInt(n[1]); l <= n; l++) {
          e.push(l);
        }
      } else {
        console.log("Malformed range (too many dashes):", f);
      }
    }
    return e;
  };
...
function ProteinGraph(h, f, k) {
  ...
  b.get_uids = function () {
    uids = [];
    for (var e = 0; e < b.dotbracket.length; e++) {
      uids.push(b.nodes[e].uid);
    }
    return uids;
  };
}
...
function RNAGraph(h, f, k) {
  ...
  b.addFakeNode = function (e) {
    ...
    for (j = 0; j < e.length; j++) {
      if (0 !== e[j] && e[j] <= b.dotbracket.length) {
        b.links.push({
        ...
        if (4 < e.length) {
          b.links.push({
            source: b.nodes[e[j] - 1],
            target:
              b.nodes[e[(j + Math.floor(e.length / 2)) % e.length] - 1],
            link_type: "fake",
            value: 2 * g,
            uid: generateUUID()
          });
    ...
    for (h = 0; h < g.length; h++) {
      for (var d = g[h], k = 0; k < d.nucs.length; k++) {
        for (var m = d.nucs[k], q = 0; q < e[m].length; q++) {
          if (!f.has(JSON.stringify([e[m][q].uid, d.uid].sort()))) {
            b.links.push({
              source: e[m][q],
              target: d,
  ...
  b.elementsToJson = function () {
    ...
    if (b.circular && b.nodes.length > 1) {
      b.links.push({
        source: b.nodes[b.nodes.length - 1], // Last Node
        target: b.nodes[0],                  // First Node
        link_type: "backbone",
        value: 1,
        uid: generateUUID()
      });
      ...
      if (0 === i % g) {
        ...
        if (1 == b.rnaLength) {
          ...
          0 !== b.pairtable[nextNode.num] &&
            ...
            (prevNode = nextNode =
              b.nodes[b.pairtable[thisNode.num] - 1]);
  ...
  for (var f = {}, k = [], b = [], e = 0; e < h.molecules.length; e++) {
    ...
    if ("rna" == g.type) {
      ...
      rg.elementsToJson()
        ...
        .reinforceStems()
        .reinforceLoops();
    } else if ("protein" == g.type) {
      rg = new ProteinGraph(g.header, g.size);
    }

    rg.addUids(g.uids);
    for (g = 0; g < rg.nodes.length; g++) {
      f[rg.nodes[g].uid] = rg.nodes[g];
    }
    k.push(rg);
  }
